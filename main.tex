...
We define the differential optical depth, and optical depth as follows
\begin{equation}
...
While one may solve the Boltzmann heirachy directly from initial conditions, it proves more instructive, intuitive and numerically effiecent to use the `line of sight method', first introduced in \cite{LoS}. We formally integrate these first order linear ODEs along the photon path, making use of an integrating factor $e^{\int d\tau (ik\mu+\kappa')} = e^{ik\mu\tau-\kappa}$ (due to the minus sign difference $\kappa'=-\frac{d\kappa}{d\tau}$), from their initial conditions up until today.  Introducing $x=k(\tau_0-\tau)$ we get  
\begin{equation}\begin{split}
