...
\end{equation}
Assuming General Relativity, it obeys the Einstein equation $G_{ab} = \frac{1}{\Mp^2} T_{ab}$, sourced by matter with energy momentum tensor $T_{ab}$. In cosmological contexts we take this to be a perfect fluid whose energy momentum tensor satisfies by homogeneity and isotropy
\begin{equation}
...
\end{equation}
where $V'=dV/d\phi$. From these we see the spacetime experiences accelerated expansion iff the potential energy dominates the kinetic: $V > \dot{\phi}^2$ and is sustained if $|\ddot{\phi}| \ll |V_{,\phi}|$. We also find by \ref{epsilon} that 
\begin{equation}
...
Now in the flat sky limit the appropriate definition of the (lensed) regular, not angular, power spectrum is
\begin{equation}
\langle T^*(\v{l})T(\v{l}')\rangle = (2\pi)^2\delta(\v{l}-\v{l}')C_l^{TT}
\end{equation}

in terms of fourier modes. To lowest order in the lensing potential power spectrum, we calculate, using the fact that $T$ and $\psi$ don't correlate, and Wick's theorem
\begin{equation}\begin{split}
...
In slow roll inflation $r$ is approximately constant, but depends slightly on $N$, precisely because $\epsilon$ varies slightly during inflation, and so $r$ varies only at second order in slow roll parameters. We can estimate  $N_{\text{eff}} \approx 30-60$ to give
\begin{equation}
...
We demonstrate the calculation alluded to above in the case of a power law potential, a large field model \cite{QBM}
\begin{equation}
